...
solve (moves, boards) = score * number_called
  where
    move_count = head [i | i <- [0..], all (has_won (take i moves)) boards]
    losing_board = head [b | b <- boards, not (has_won (take (move_count - 1) moves) b)]
    score = sum [x | x <- concat losing_board, not (elem x (take move_count moves))]
